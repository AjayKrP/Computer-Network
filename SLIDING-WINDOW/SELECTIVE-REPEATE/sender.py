...
def divide_file(file,n): # going to divide the file into n packets, each packet except last has size(file)/n size and last packet has different size
    ...
    content = f.read(size_file - ((n-1)*(packet_size))) # To obtain last packet
    packet_list.append(content)
    print (packet_list)
...
def Main():
    ...
    s_ack.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    file1 = 'sender.py'#raw_input("Enter File Name: ")
    w = 4#int(raw_input("Enter Window Size: "))
    timeout = 1.0#float(raw_input("Enter Timeout Time: "))
    n = 10 #int(raw_input("Enter Number of Packets: "))
    print
    if w<=0:
        ...
        if presenttime - timee > timeout:
            ...
            for j in range(win_base,win_base+w):
                if(sent_flag[j] == 0):
                    ...
                    lost_flag.append(j)
            for j in range(len(lost_flag)):
                print("resending packet: ", sent_flag[lost_flag[j]])
                if sent_flag[lost_flag[j]] == 1:
                    #print('lost packet sequence no: '+sent_flag[lost_flag[j]])
                    send_packet(lost_flag[j], s)
                    s.recv(1)
                else:
                    print('Error! can\'t sent packet...')
            timee = time.time()
            continue
